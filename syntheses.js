...
async function calculateTotalSynthesis() {
    ...
    Object.keys(data).forEach(trackKey => {
        // Ignorer les métadonnées (updatedAt, etc.)
        if (trackKey === 'updatedAt') return;
        
        // Les clés sont au format: "week_X_Y-Jour-Créneau-itemId"
        // On cherche l'itemId à la fin
        const parts = trackKey.split('-');
        if (parts.length >= 4) {
            // L'itemId est la dernière partie (ou plusieurs parties si l'ID contient des tirets)
            const itemId = parts.slice(3).join('-');
            
            // Vérifier si cet item existe
            const item = allItems.find(i => i.id === itemId);
            if (item && totals.hasOwnProperty(itemId)) {
...
function getWeekKeyForDate(date) {
    // Calculer le nombre de semaines depuis/jusqu'à aujourd'hui
    const today = new Date();
    
    // Mettre les dates à minuit pour un calcul précis
    ...
    // Calculer le weekOffset (nombre de semaines depuis aujourd'hui)
    // 0 = cette semaine, -1 = semaine dernière, +1 = semaine prochaine
...
function getWeekTypeForDate(date) {
    ...
    // Mettre les deux dates à minuit
